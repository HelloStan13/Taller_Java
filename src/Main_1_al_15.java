import Ejercicio1.Ejercicio1y2;
import Ejercicio_10.Ejercicio_10;
import Ejercicio_11.Ejercicio_11;
import Ejercicio_12.Ejercicio_12;
import Ejercicio_13.Ejercicio_13;
import Ejercicio_14.Ejercicio_14;
import Ejercicio_15.Ejercicio_15;
import Ejercicio_3.Ejercicio_3;
import Ejercicio_4.Ejercicio_4;
import Ejercicio_5y6.Ejercicio_5y6;
import Ejercicio_7.Ejercicio_7;
import Ejercicio_8.Ejercicio_8;
import Ejercicio_9.Ejercicio_9;

import java.util.Scanner;

public class Main_1_al_15 {
    public static void main(String[] args) {

        Scanner sc = new Scanner(System.in);

        Ejercicio1y2 ejercicio1y2 = new Ejercicio1y2();
        ejercicio1y2.Practica1();

        Ejercicio_3 ejercicio_3 = new Ejercicio_3();
        ejercicio_3.Practica_3();

        Ejercicio_4 ejercicio_4 = new Ejercicio_4();
        ejercicio_4.Practica_4();

        Ejercicio_5y6 ejercicio_5 = new Ejercicio_5y6();
        ejercicio_5.Practica_5();

        Ejercicio_5y6 ejercicio_6 = new Ejercicio_5y6();
        ejercicio_6.Practica_6();

        Ejercicio_7 ejercicio_7 = new Ejercicio_7();
        ejercicio_7.Practica_7();

        Ejercicio_8 ejercicio_8 = new Ejercicio_8();
        ejercicio_8.Practica_8();

        Ejercicio_9 ejercicio_9 = new Ejercicio_9();
        ejercicio_9.Practica_9();

        Ejercicio_10 ejercicio_10 = new Ejercicio_10();
        ejercicio_10.Practica_10();

        Ejercicio_11 ejercicio_11 = new Ejercicio_11();
        ejercicio_11.Practica_11();

        Ejercicio_12 ejercicio_12 = new Ejercicio_12();
        ejercicio_12.Practica_12();

        Ejercicio_13 ejercicio_13 = new Ejercicio_13();
        ejercicio_13.Practica_13();

        Ejercicio_14 ejercicio_14 = new Ejercicio_14();
        ejercicio_14.Practica_14();

        Ejercicio_15 ejercicio_15 = new Ejercicio_15();
        ejercicio_15.Practica_15();

    }
}
